...
public class LeveldbTimelineStore implements TimelineStore {
    ...
    public TimelineEntity getEntity(String entityId, String entityType,
                                    EnumSet<TimelineReader.Field> fields) throws IOException {
        Long revStartTime = getStartTimeLong(entityId, entityType);
        if (revStartTime == null) {
            return null;
        }
        byte[] prefix = KeyBuilder.newInstance().add(ENTITY_ENTRY_PREFIX)
                .add(entityType).add(GenericObjectMapper.writeReverseOrderedLong(revStartTime))
    ...
    private static TimelineEntity getEntity(String entityId, String entityType,
                                            Long startTime, EnumSet<TimelineReader.Field> fields, DBIterator iterator,
    ...
    public TimelineEvents getEntityTimelines(String entityType,
        ...
        try {
            ...
            for (Map.Entry<byte[], List<EntityIdentifier>> entry :
                ...
                for (EntityIdentifier entityIdentifier : entry.getValue()) {
                    ...
                    if (windowStart != null) {
                        last = KeyBuilder.newInstance().add(prefix)
                                .add(GenericObjectMapper.writeReverseOrderedLong(windowStart)).getBytesForLookup();
                    }
                    if (limit == null) {
                        limit = DEFAULT_LIMIT;
                    }
        ...
        try {
            ...
            if (fromId != null) {
                ...
                if (fromIdStartTime <= endtime) {
                    ...
                    first = kb.add(GenericObjectMapper.writeReverseOrderedLong(fromIdStartTime))
            ...
            if (starttime != null) {
                ...
                last = KeyBuilder.newInstance().add(base).add(entityType)
                        .add(GenericObjectMapper.writeReverseOrderedLong(starttime)).getBytesForLookup();
            }
            if (limit == null) {
    ...
    private void put(TimelineEntity entity, TimelinePutResponse response) {
        ...
        try {
            ...
            revStartTime = GenericObjectMapper.writeReverseOrderedLong(startAndInsertTime
                    .startTime);
            ...
            if (relatedEntities != null && !relatedEntities.isEmpty()) {
                for (Map.Entry<String, Set<String>> relatedEntityList :
            ...
            if (primaryFilters != null && !primaryFilters.isEmpty()) {
                for (Map.Entry<String, Set<Object>> primaryFilter :
            ...
            if (otherInfo != null && !otherInfo.isEmpty()) {
                for (Map.Entry<String, Object> i : otherInfo.entrySet()) {
            ...
            try {
                StartAndInsertTime relatedEntityStartAndInsertTime =
                        getAndSetStartTime(relatedEntity.getId(), relatedEntity.getType(),
                                GenericObjectMapper.readReverseOrderedLong(revStartTime, 0), null);
                if (relatedEntityStartAndInsertTime == null) {
                    throw new IOException("Error setting start time for related entity");
                }
                byte[] relatedEntityStartTime = GenericObjectMapper.writeReverseOrderedLong(
                ...
                db.put(createEntityMarkerKey(relatedEntity.getId(),
                        relatedEntity.getType(), relatedEntityStartTime),
                        GenericObjectMapper.writeReverseOrderedLong(relatedEntityStartAndInsertTime
                                .insertTime));
            } catch (IOException e) {
    ...
    private StartAndInsertTime checkStartTimeInDb(EntityIdentifier entity,
        ...
        if (v == null) {
            ...
            startAndInsertTime = new StartAndInsertTime(GenericObjectMapper.readReverseOrderedLong(v, 0),
                    GenericObjectMapper.readReverseOrderedLong(v, 8));
        }
